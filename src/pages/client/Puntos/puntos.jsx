    ...
    >
      <div
        className={`
          ...
          ${isUnlocked ? `${iconColor} ${iconBg}` : 'bg-gray-200 text-gray-400'}
        `}
      >
      ...
      <div className="flex-1">
        <h3
          className={`font-semibold text-sm mb-1 ${
            isUnlocked ? textColor : 'text-gray-500'
          }`}
        >
          {name}
        </h3>
...
const GamificacionPerfil = () => {
  ...
  const fetchInsigniasData = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(
          response.data.message || 'No se pudieron cargar las insignias.'
        );
      }
    } catch (err) {
  ...
  const getBadgeTemplate = (badgeName, nivel, isUnlocked) => {
    ...
    if (isUnlocked) {
      ...
      const badgeFromAll = allBadges[nivel.toLowerCase()]?.find(
  ...
  const renderBadgeSection = (nivel, title) => {
    ...
    const desbloqueadasCount = insigniasDelNivel.filter(
    ...
    return (
      <div className="mb-6">
        <div className="flex items-center space-x-3 mb-3">
          <div
            className={`w-1 h-6 rounded-full ${unlockedTemplates[nivel]?.bgColor || 'bg-gray-300'}`}
          ></div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {insigniasDelNivel.map((insignia) => {
            const badgeTemplate = getBadgeTemplate(
              insignia.nombre,
              nivel,
      ...
      <div className="max-w-6xl mx-auto mb-6">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600 max-w-2xl mx-auto">
            Gana puntos, desbloquea insignias y disfruta de beneficios
            exclusivos
          </p>
        </div>
      </div>
            ...
            <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 shadow-sm">
              ...
              <div className="relative">
                ...
                <div
                  ...
                  style={{
                    left: `calc(${Math.min(resumen.progresoTotal, 97)}% - 8px)`,
                  }}
                >
                  <div className="relative">
              ...
              <div className="flex justify-between items-center mt-2 text-xs">
                <span className="text-green-700 font-medium">
                  {resumen.totalDesbloqueadas} de{' '}
                  {resumen.totalDesbloqueadas + resumen.totalPorConseguir}{' '}
                  insignias
                </span>
                <span className="text-green-600">
                  {resumen.totalPorConseguir > 0
                    ? `${resumen.totalPorConseguir} restantes`
                    : '¡Completado!'}
                </span>
              </div>
            </div>
