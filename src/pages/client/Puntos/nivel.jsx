            ...
            <div className="text-center mb-6">
              ...
              <div className="bg-amber-800/20 rounded-xl p-3 border border-amber-700/30">
                ...
                <p className="text-xs opacity-90 text-amber-900 font-medium">
                  {levelName === 'Anfitrión' &&
                    '5% de descuento en todas tus rentas'}
                  {levelName === 'Organizador Pro' &&
                    '10% de descuento + Prioridad en entregas'}
                  {levelName === 'Embajador de Fiesta' &&
                    '15% de descuento + Envío gratis + Soporte prioritario'}
                </p>
              </div>
            </div>
  ...
  return (
    <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200">
      <div className="text-center mb-5">
        <div
          className={`inline-flex items-center justify-center w-20 h-20 ${currentLevel.bgColor} ${currentLevel.borderColor} border-2 rounded-full mb-3 overflow-hidden`}
        >
          <img
      ...
      <div className="space-y-2">
        ...
        {levels.map((level) => {
          ...
          return (
            ...
            >
              <div
                className={`flex-shrink-0 w-8 h-8 rounded-full bg-white border border-gray-200 overflow-hidden mr-3 ${
                  !isUnlocked ? 'relative' : ''
                }`}
              >
                {isUnlocked ? (
                ...
                <div className="absolute left-0 top-full mt-2 hidden group-hover:block z-10 w-48">
                  <div className="bg-gray-900 text-white text-xs rounded-lg p-3 shadow-lg">
                    ...
                    <div className="text-gray-200">
                      {level.name === 'Invitado' &&
                        'Acceso al programa de Puntos y Logros'}
                      {level.name === 'Anfitrión' &&
                        '5% de descuento en todas tus rentas'}
                      {level.name === 'Organizador Pro' &&
                        '10% de descuento + Prioridad en las rutas de entrega'}
                      {level.name === 'Embajador de Fiesta' &&
                        '12% de descuento en todas las rentas + Prioridad en las rutas de entrega + Envío gratuito sin mínimo de compra'}
                    </div>
    ...
    <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="bg-green-50 rounded-lg p-3 text-center border border-green-200">
          <FontAwesomeIcon
            icon={faArrowDown}
            className="text-green-600 mb-1 text-sm"
          />
          ...
          {pointEarningMethods.map((method) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600">{method.points}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    const fetchGamificacionData = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          console.log(
            'Nivel anterior (sessionStorage):',
            previousLevelData?.currentLevelName
          );
          ...
          console.log(
            '💾 Guardando nuevo nivel para próxima comparación:',
            newData.currentLevelName
          );
          setPreviousLevel({
          ...
          throw new Error(
            response.data.message || 'No se pudieron cargar los datos.'
          );
        }
      } catch (err) {
