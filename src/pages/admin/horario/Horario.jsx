...
const Horario = () => {
  ...
  const [editForm, setEditForm] = useState({ open: "", close: "", isClosed: false });
  ...
  const handleSaveEdit = async (day) => {
    if (!editForm.isClosed && (!editForm.open || !editForm.close)) {
      setError("Por favor, completa las horas de apertura y cierre, o marca el día como cerrado.");
      return;
    }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-md mx-auto  dark:from-gray-800 dark:to-gray-900 rounded-3xl  border border-gray-200 dark:border-gray-700">
      ...
      <h1 className="text-2xl sm:text-3xl font-extrabold mb-6 text-gray-900 dark:text-gray-100 flex items-center justify-center">
        <FontAwesomeIcon icon={faClock} className="mr-3 text-yellow-600 animate-spin-slow" />
        Horario de Atención
      </h1>
      ...
      {loading && (
        <div className="mb-4 flex justify-center">
          <FontAwesomeIcon icon={faClock} className="text-yellow-600 text-2xl animate-spin" />
        </div>
      )}
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {schedule.open && schedule.close ? (
                          ...
                          <span className="flex items-center">
                            <FontAwesomeIcon icon={faTimesCircle} className="mr-1 text-red-500" />
                            Cerrado
                          </span>
                        )}
                      </p>
