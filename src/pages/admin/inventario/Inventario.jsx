...
const Inventatio = ({ onNavigate, setDatosInventario }) => {
  ...
  */
  ...
  const generatePreview = () => {
    if (inventory.length > 0) {
      ...
      const groupedByProduct = inventory.reduce((acc, item) => {
        ...
        if (
          !acc[item.idProducto].colores.some(
            (c) => c.color === (item.colores || 'Sin color')
          )
        ) {
        ...
        <tbody>
          ...
          <tr style="background-color: #e0e0e0; font-weight: bold;">
            ...
            <td colspan="3" style="border: 1px solid #ddd; padding: 15px; font-size: 14px;">${mainInventoryFiltered
              .reduce(
                (sum, item) =>
                  sum + item.stockReal * (item.precioAlquiler || 0),
                0
              )
              .toFixed(2)} MXN</td>
          </tr>
      ...
      <p style="font-size: 12px; color: #666;"><strong>Importe con letra:</strong> ${numberToWords(
        mainInventoryFiltered
          .reduce(
            (sum, item) => sum + item.stockReal * (item.precioAlquiler || 0),
            0
          )
          .toFixed(2)
      )} pesos (la suma de la cantidad física multiplicada por el precio de todos los productos).</p>
    `;
  ...
  const generatePDF = () => {
    ...
    doc.text(
      'Av. San Luis Potosí #58, Col. Tahuizan, Huejutla de Reyes Hidalgo - México',
      textStartX,
      32
    );
    ...
    const tableData = Object.values(groupedByProduct).map((product) => [
      ...
      product.precioAlquiler
        ? `$${Number(product.precioAlquiler).toFixed(2)}`
        : 'N/A',
      new Date(product.fechaRegistro).toLocaleDateString('es-MX', {
        timeZone: 'America/Mexico_City',
      }),
    ]);

    // --- Calcular total ---
    const total = mainInventoryFiltered
      .reduce((sum, item) => {
        const precio = !isNaN(Number(item.precioAlquiler))
          ? Number(item.precioAlquiler)
          : 0;
    ...
    doc.text(
      `Notas: Este reporte refleja el estado del inventario al ${currentDate}.`,
      20,
      finalY
    );
    doc.text(
      `Importe con letra: ${numberToWords(total)} pesos (valor total de la cantidad física x precio).`,
      20,
      finalY + 8
    );
      ...
      {Object.keys(aggregatedSecondaryByBodega).length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg">
          <div className="flex flex-col justify-center items-center py-16 animate-fade-in-up">
            ...
            <p className="text-gray-400 dark:text-gray-500 mt-2">
              Los inventarios secundarios aparecerán aquí cuando agregues
              productos
            </p>
          </div>
        </div>
        ...
        Object.keys(aggregatedSecondaryByBodega).map((bodega) => {
          ...
          return (
            <div key={bodega} className="mb-12">
              <div
                className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden ${bodegaInactiva ? 'opacity-75' : ''}`}
              >
                {/* Header del inventario secundario */}
                <div
                  className={`px-6 py-4 relative ${bodegaInactiva ? 'bg-gradient-to-r from-red-500 to-red-600' : 'bg-gradient-to-r from-blue-500 to-blue-600'}`}
                >
                  <h2 className="text-3xl font-bold text-white text-center flex items-center justify-center gap-3">
                ...
                <div
                  className={`p-6 ${bodegaInactiva ? 'bg-red-50 dark:bg-red-900/10' : ''}`}
                >
                  <div className="overflow-x-auto animate-fade-in-up">
                    <table className="w-full">
                      <thead>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md shadow-2xl transform transition-all duration-300 ease-in-out hover:shadow-3xl">
            ...
            selectedItem.variants.length > 1 ? (
              ...
              <div className="mb-5">
                <label className="block text-gray-700 dark:text-gray-300 mb-2 font-medium">
                  {selectedItem?.colores ? `${selectedItem.colores} - ` : ''}
                  Nuevo Stock:
                </label>
                <input
                  type="number"
