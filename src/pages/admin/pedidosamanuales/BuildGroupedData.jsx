...
export function buildGroupedData(productosDisponibles) {
    ...
    productosDisponibles.forEach((p) => {
        ...
        if (p.estado !== "activo" || !p.precioAlquiler || parseFloat(p.precioAlquiler) <= 0) return;
    ...
    grouped[catId].productos[prodId].colorMap[colorKey].stock += parseInt(p.stock, 10) || 0;
  });
    
  return Object.values(grouped).map((cat) => {
    const productosArray = Object.values(cat.productos).map((prod) => {
      const colorOptions = Object.entries(prod.colorMap).map(([color, data]) => ({
        color,
        stock: data.stock,
