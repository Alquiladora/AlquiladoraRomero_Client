...
const paymentColors = isDark ? paymentColorsDark : paymentColorsLight;
  ...
  const metrics = useMemo(() => {
    ...
    const totalRevenue = ordersData.reduce((sum, o) => sum + (Number(o.pago?.totalPagado) || 0), 0);
  ...
  const dailyRevenueData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayRevenue = ordersData
        .filter(o => o.fechas?.inicio && new Date(o.fechas.inicio).toISOString().split("T")[0] === dateStr)
  ...
  const topProducts = useMemo(() => {
    ...
    return Object.entries(counts)
      .map(([name, data]) => ({ name, cantidad: data.count, ingresos: data.revenue }))
  ...
  const MetricCard = ({ icon: Icon, title, value, subtitle, color = "blue" }) => (
    <div className={`bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md flex items-center space-x-4 hover:shadow-xl transition-transform transform hover:-translate-y-1`}>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
        <MetricCard icon={Package} title="Total Pedidos Activos" value={metrics.total} color="yellow" />
        <MetricCard icon={AlertTriangle} title="Pedidos con Pagos Pendientes" value={pedidosPendientesPago.length} color="red" />
        <MetricCard icon={DollarSign} title="Ingresos Totales" value={`$${metrics.totalRevenue.toLocaleString()}`} color="green" />
        <MetricCard icon={Clock} title="Potencial Ingresos (Total Deuda)" value={`$${totalDeudaPendiente.toLocaleString()}`} color="amber" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
          <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Estado de Pedidos Activos</h3>
        ...
        {/* Métodos pago */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
          <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Métodos de Pago</h3>
          <ResponsiveContainer width="100%" height={300}>
        ...
        {/* Pedidos por cliente */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
          <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Pedidos por Cliente (Top 5)</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={topClients} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="cliente" tick={{ fontSize: 12 }} interval={0} angle={-30} textAnchor="end" />
              <YAxis />
              <Tooltip content={<CustomTooltip />} />
  ...
  <div className="bg-gray-50 dark:bg-gray-900 rounded-2xl shadow-md p-6">
    ...
    <div className="space-y-4 max-h-[320px] overflow-y-auto">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mt-12 max-w-7xl mx-auto">
        <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Pedidos Recientes</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Método Pago</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Total</th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {ordersData.slice(0, 5).map(order => (
                <tr key={order.idPedido} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">#{order.idPedido}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{order.cliente?.nombre || "Sin nombre"}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                   
                   <span
  className="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
  style={{
    backgroundColor: (statusColors[order.estado?.toLowerCase()] || '#6B7280') + "20", // transparencia
    color: statusColors[order.estado?.toLowerCase()] || '#6B7280',
  }}
>
  {order.estado}
</span>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{order.pago?.formaPago || "N/A"}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">${Number(order.pago?.totalPagado || 0).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
