...
export function validateProduct(product, totalImagesCount) {
  ...
  if (!product.nombre || product.nombre.trim().length < 3) {
    ...
    const distinctChars = new Set(
      product.nombre.toLowerCase().replace(/\s/g, '')
    );
    if (distinctChars.size < 3) {
      errors.nombre =
        'El nombre no parece válido. Ingrese un valor descriptivo.';
    }
  }
  ...
  if (!product.color || product.color.trim() === '') {
    ...
    for (let i = 0; i < colores.length; i++) {
      ...
      if (c.length < 3) {
        errors.color =
          "Cada color debe tener al menos 3 caracteres. Ej: 'Rojo, Azul, Verde'";
        break;
      }
      if (!REGEX_NOMBRE.test(c)) {
        errors.color =
          'Solo se permiten letras y espacios en cada color (mín. 3 caracteres).';
        break;
      }
      if (new Set(c.toLowerCase()).size < 3) {
        errors.color =
          'El color ingresado no parece válido. Por favor, ingresa un color descriptivo.';
        break;
      }
    }
  }
  ...
  if (!product.material || product.material.trim() === '') {
    ...
    errors.material =
      'El material contiene caracteres no permitidos. Ej: letras, números, espacios, guiones...';
  } else {
    const distinctMat = new Set(
      product.material.toLowerCase().replace(/\s/g, '')
    );
    if (distinctMat.size < 3) {
      errors.material =
        'El material no parece válido. Ingrese un valor descriptivo.';
    }
  }
