...
const UpdateStatusModal = ({ pedido, onClose, onUpdateStatus }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        'Estado actualizado correctamente. El stock ha sido restaurado en el inventario.'
      );
    } catch (error) {
...
const GestionPedidosDevueltos = ({ onNavigate }) => {
  ...
  const filteredPedidos = pedidos
    .filter(
      (pedido) => filterEstado === 'Devuelto' || pedido.estado === filterEstado
    )
    .filter((pedido) => {
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 dark:bg-gray-800">
        <div className="flex justify-between items-center bg-[#fcb900] p-4 rounded-t-xl">
          <h2 className="text-xl font-bold text-white flex items-center space-x-2">
            ...
            <span>
              Detalles del Pedido #{showDetailsModal?.idPedido || 'N/A'}
            </span>
          </h2>
          <button
                      ...
                      {currentPedidos.map((pedido, index) => (
                        ...
                        >
                          ...
                          <td className="px-2 sm:px-4 py-2 text-sm text-gray-800 dark:text-gray-200">
                            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold shadow-sm select-none bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                          ...
                          <td className="px-2 sm:px-4 py-2 text-sm flex space-x-2">
                            ...
                            <button
                              onClick={() =>
                                handleShowUpdateStatusModal(pedido)
                              }
                  ...
                  <div className="flex items-center space-x-2 flex-wrap justify-center">
                    ...
                    {totalPages <= 7 ? (
                      ...
                      <>
                        ...
                        {currentPage > 4 && (
                          <span className="px-2 sm:px-3 py-1 text-gray-600 dark:text-gray-300">
                            ...
                          </span>
                        )}
