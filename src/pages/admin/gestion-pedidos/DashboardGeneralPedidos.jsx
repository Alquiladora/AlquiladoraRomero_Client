...
const SummaryCard = ({ icon, label, value, className = '' }) => (
  <div
    className={`dark:text-gray-200 dark:bg-gray-800 dark:border-gray-700 border border-gray-200 p-3 sm:p-4 md:p-5 rounded-lg sm:rounded-xl shadow-sm hover:shadow-md transition-all duration-200 flex items-center space-x-3 sm:space-x-4 overflow-hidden ${className}`}
  >
    <FontAwesomeIcon
      icon={icon}
      className="text-lg sm:text-xl md:text-2xl text-blue-500 flex-shrink-0"
    />
    <div className="min-w-0 flex-1">
      <p className="text-xs sm:text-sm font-medium uppercase text-gray-500 dark:text-gray-400 leading-tight break-words">
        {label}
      </p>
      <p className="text-base sm:text-lg md:text-xl font-bold text-gray-900 dark:text-white leading-tight break-words">
        {value}
      </p>
    </div>
  </div>
...
const PedidosGeneralesDashboard = () => {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const response = await api.get(
        `/api/pedidos/dashboard-stats?${params.toString()}`,
        {
      ...
      if (result.success && result.stats) {
        ...
        setStats({
          ...
          totalRevenueFinalized:
            Number(fetchedStats.totalRevenueFinalized) || 0,
          ...
          clientTypeCounts:
            fetchedStats.clientTypeCounts || defaultStats.clientTypeCounts,
          ...
          topProducts: Array.isArray(fetchedStats.topProducts)
            ? fetchedStats.topProducts
            : [],
          ...
          topClients: Array.isArray(fetchedStats.topClients)
            ? fetchedStats.topClients
            : [],
        });
      } else {
        toast.error(
          'No se encontraron datos para el año seleccionado o hubo un error.'
        );
  ...
  const hasData =
    stats.totalOrders > 0 || stats.totalRevenue > 0 || stats.uniqueClients > 0;

  if (loading) {
    return <CustomLoading />;
  }

  const averageValue = stats.totalOrders
  ...
  return (
    <div
      ...
      className="min-h-screen p-2 sm:p-4 md:p-6 lg:p-8 bg-gray-50 dark:bg-gray-900 transition-colors duration-200"
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-violet-50 rounded-xl p-4 border border-purple-100">
          <h3 className="font-semibold text-purple-800 mb-3 text-sm uppercase tracking-wide">
            Ingresos
          </h3>
          <div className="space-y-3">
            <SummaryCard
      ...
      {!hasData && (
        <div className="col-span-full text-center py-8 text-gray-500 dark:text-gray-400 text-sm sm:text-base font-medium">
          No hay datos disponibles para el año {selectedYear} y estado "
          {selectedStatus}".
        </div>
      )}
      ...
      {hasData && (
        <>
          <section className="mb-6 sm:mb-8">
            <h2 className="text-sm sm:text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4 sm:mb-6 px-2">
              Métricas Mensuales e Insights
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <h3 className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200 mb-3 break-words">
                  ...
                  options={{
                    ...
                    yaxis: {
                      ...
                      labels: {
                        ...
                        formatter: (val) =>
                          `$${Number(val || 0).toLocaleString('es-MX', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`,
                      },
                    },
                    plotOptions: {
                      bar: {
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden md:col-span-1 lg:col-span-1">
                ...
                <Chart
                  ...
                  series={[
                    stats.payCounts.pendiente,
                    stats.payCounts.completado,
                  ]}
                  type="pie"
                  height={250}
                />
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden md:col-span-1 lg:col-span-1">
                ...
                <Chart
                  ...
                  series={Object.values(stats.clientTypeCounts).map((v) =>
                    Number(v)
                  )}
                  type="pie"
                  height={250}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <Chart
                  ...
                  series={Object.values(stats.statusCounts).map((v) =>
                    Number(v)
                  )}
                  type="donut"
                  height={250}
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <Chart
                  ...
                  series={[
                    {
                      name: 'Pedidos',
                      data: Object.values(stats.durationBins).map((v) =>
                        Number(v)
                      ),
                    },
                  ]}
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden lg:col-span-2 xl:col-span-1">
                ...
                <Chart
                  options={{
                    ...
                    xaxis: {
                      ...chartCommon.xaxis,
                      categories: stats.topProducts
                        .slice(0, 5)
                        .map((p) => p?.nombre?.substring(0, 30) || 'N/A'),
                      labels: { ...chartCommon.xaxis.labels, rotate: 0 },
                    },
              ...
              <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <Chart
                  options={{
                    ...
                    xaxis: {
                      ...chartCommon.xaxis,
                      categories: stats.topClients
                        .slice(0, 5)
                        .map((c) => c?.clientName?.substring(0, 25) || 'N/A'),
                      labels: {
                        ...chartCommon.xaxis.labels,
                        rotate: -30,
                        trim: true,
                      },
                    },
                ...
                <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden lg:col-span-1">
                  ...
                  <ul className="space-y-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400 max-h-48 overflow-y-auto">
                    {stats.topClients.slice(5, 10).map((client, idx) => (
                      <li
                        key={idx}
                        className="flex justify-between break-words"
                      >
                        <span className="truncate max-w-[60%]">
                          {client?.clientName?.substring(0, 30) || 'N/A'}
                        </span>
                        <span className="font-medium min-w-[40%] text-right">
                          $
                          {Number(client?.revenue || 0).toLocaleString('es-MX')}
                        </span>
                      </li>
                    ))}
                  </ul>
