      ...
      if (response.data.message === "Token válido. Puede proceder con el cambio de contraseña. El token ha sido eliminado.") {
        ...
        setTimeout(() => {
          if (user && user.rol) {
            switch (user.rol) {
              case "cliente":
                navigate("/cliente/updatePass", { state: { idUsuario: user.idUsuarios } });
                break;
              case "administrador":
                navigate("/administrador/updatePass", { state: { idUsuario: user.idUsuarios } });
                break;
              case "repartidor":
                navigate("/repartidor/updatePass", { state: { idUsuario: user.idUsuarios } });
                break;
              default:
                navigate("/login");
                break;
    ...
    const timer = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          ...
          showAlert("error", "El token ha expirado. Vuelve a intentarlo más tarde.");
          return 0;
        }
        return prev - 1;
      });
...
export default TokenModalAdmin;
