    ...
    try {
      ...
      while (true) {
        const response = await api.get(
          `/api/pedidos/pedidos/detalles/pagos?page=${page}`,
          {
        ...
        {isLoading ? (
          ...
          <>
            <div className="mb-4 text-sm text-gray-600 dark:text-gray-300">
              {totalItems} pedidos{' '}
              {activeFilters > 0 && `(filtrados: ${orders.length})`}
            </div>
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-600">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-green-50 dark:bg-green-900/50">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-200 uppercase"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                  {orders.map((order) => {
                    const metodo =
                      order.pagos?.listaPagos?.[0]?.metodoPago || 'N/A';
                    const forma =
                      order.pagos?.listaPagos?.[0]?.formaPago || 'N/A';
                    return (
                      <tr
                        key={order.idPedido}
              ...
              <div className="flex justify-between items-center mt-6">
                ...
                <div className="flex space-x-2">
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    const pageNum =
                      currentPage > 3 ? totalPages - 4 + i : i + 1;
                    return (
                      <button
                        key={i}
