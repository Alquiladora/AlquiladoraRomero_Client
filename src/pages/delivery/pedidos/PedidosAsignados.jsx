...
import { AiOutlineCar, AiOutlineShoppingCart, AiOutlineUndo, AiOutlineWarning, AiFillExclamationCircle } from 'react-icons/ai';
...
const IncidentFormModal = ({ isOpen, onClose, order, onSubmit }) => {
  ...
  const handleProductIssueChange = (productId, field, value) => {
    setProductIssues((prev) => {
      const selectedIds = prev.filter((issue) => issue.selected).map((issue) => issue.id);
      return prev.map((issue) =>
        selectedIds.includes(issue.id)
          ? {
              ...issue,
              [field]: field === 'issueQuantity'
                ? Math.min(Number(value) || 0, order.products.find((p) => p.id === issue.id)?.quantity || 0)
                : value,
            }
          : issue
      );
  ...
  const validateInputs = () => {
    ...
    if (!['En alquiler', 'Devuelto'].includes(orderStatus)) {
      if (entireOrderIssue && !orderObservations.trim()) {
        setError('Por favor, proporcione observaciones para el incidente del pedido.');
        return false;
      }
      if (!entireOrderIssue) {
        const selectedIssues = productIssues.filter((issue) => issue.selected);
        if (selectedIssues.length === 0) {
          setError('Por favor, seleccione al menos un producto para reportar el incidente.');
          return false;
        }
        for (const issue of selectedIssues) {
          const product = order.products.find((p) => p.id === issue.id);
          if (issue.status === 'Incompleto') {
            if (issue.issueQuantity <= 0 || issue.issueQuantity > product.quantity) {
              setError(`La cantidad afectada para ${product.name} debe estar entre 1 y ${product.quantity}.`);
              return false;
            }
            if (!issue.observations.trim()) {
              setError(`Por favor, proporcione observaciones para el producto ${product.name}.`);
              return false;
            }
          } else if (issue.status === 'Incidente' && !issue.observations.trim()) {
            setError(`Por favor, proporcione observaciones para el producto ${product.name}.`);
            return false;
          }
        }
      ...
      productIssues: entireOrderIssue
        ...
        : productIssues
            ...
            .map((issue) => ({
              id: issue.id,
              estado: issue.status,
              cantidad_afectada: issue.status === 'Incompleto' ? issue.issueQuantity : 0,
              nota: issue.status === 'Incompleto' ? `${issue.issueQuantity} ${issue.observations}` : issue.observations,
            })),
  ...
  <div className="space-y-6">
    <div className="flex flex-wrap items-center gap-2">
      ...
      <span className="text-sm text-gray-500">
        ({productIssues.filter(p => p.selected).length} seleccionados)
      </span>
    </div>

    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {order.products.map((product) => {
        const issue = productIssues.find((p) => p.id === product.id);
              ...
              <div className="flex gap-4 w-full overflow-hidden">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-sm text-gray-600 flex flex-wrap gap-1 mt-1">
                    ...
                    <span></span>
                    <span>Precio${product.unitPrice.toFixed(2)}</span>
                    <span></span>
                    <span>Subtotal: ${product.subtotal.toFixed(2)}</span>
                    <span></span>
                    <span className="text-gray-500">Color: {product.color || 'N/A'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-gray-200">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleProductIssueChange(product.id, 'status', e.target.value)}
                  ...
                  >
                    {['Disponible', ...availableStatuses].map((status) => (
                ...
                {issue.status === 'Incompleto' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleProductIssueChange(product.id, 'issueQuantity', e.target.value)
                      }
                ...
                {/* Observaciones */}
                {(issue.status === 'Incidente' || issue.status === 'Incompleto') && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Observaciones
                    </label>
                    <textarea
                      ...
                      onChange={(e) => handleProductIssueChange(product.id, 'observations', e.target.value)}
                      ...
                      rows={3}
                      placeholder={`Describe el ${issue.status === 'Incidente' ? 'incidente' : 'motivo del faltante'}...`}
                    />
                  </div>
                )}
              </div>
...
// OrderCard Component
const OrderCard = ({ order, onStatusChange, onViewDetails, onCancel }) => {
  const statusStyles = {
    Enviando: {
  ...
  const paymentProgress = order.totalToPay > 0 ? (order.totalPaid / order.totalToPay) * 100 : 0;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
      <div className={`h-1 ${order.type === 'entrega' ? 'bg-emerald-500' : 'bg-blue-500'}`} />
      <div className="p-4 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
          <div className="flex items-center flex-wrap gap-3">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm font-medium ${statusStyle.bg} ${statusStyle.color}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => onViewDetails(order)} className="text-blue-600 dark:text-blue-400 hover:underline text-sm font-medium">
              Ver detalles
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <div className="flex items-center gap-2">
              ...
              <span className="truncate">{order.locality}, {order.municipality}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="h-4 w-4 text-slate-400" />
              <span>Ocupación: {new Date(order.deliveryDate).toLocaleDateString('es-MX')}</span>
            </div>
          </div>
          <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
            <div className="flex items-center gap-2">
        ...
        <div className="mb-4">
          {isFullyPaid ? (
            <div className="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-900 border border-emerald-200 dark:border-emerald-600 rounded-lg">
              ...
              <span className="font-semibold text-emerald-800 dark:text-emerald-300">PAGADO COMPLETO</span>
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg">
              ...
              <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
                <span>Restante: ${(order.totalToPay - order.totalPaid).toFixed(2)}</span>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
          <div className="flex-1">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Cambiar estado</label>
            <select
              value={normalizedStatus}
              onChange={(e) => onStatusChange(order.id, capitalizeStatus(e.target.value))}
            ...
            >
              <option value="" disabled>
                {availableStatuses.length === 0 ? 'No hay estados disponibles' : 'Selecciona nuevo estado'}
              </option>
              {availableStatuses.map((status) => (
...
const OrderDetailsModal = ({ isOpen, onClose, order }) => {
  ...
  const statusStyles = {
    Disponible: { color: 'text-green-600', bg: 'bg-green-100', border: 'border-green-200' },
    Incompleto: { color: 'text-yellow-700', bg: 'bg-yellow-200', border: 'border-yellow-300' },
    ...
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white">Detalles del Pedido #{order.id}</h2>
          <button onClick={onClose} className="text-white hover:bg-orange-700 rounded-full p-2">
            <X className="h-6 w-6" />
          </button>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-slate-600 dark:text-slate-300">Descripción</p>
              <p className="font-semibold text-slate-900 dark:text-white">{order.description}</p>
            </div>
            <div>
              <p className="text-sm text-slate-600 dark:text-slate-300">Ubicación</p>
              <p className="font-semibold text-slate-900 dark:text-white">{order.locality}, {order.municipality}, {order.state}</p>
            </div>
            <div>
              <p className="text-sm text-slate-600 dark:text-slate-300">Fecha</p>
              <p className="font-semibold text-slate-900 dark:text-white">{new Date(order.deliveryDate).toLocaleDateString('es-MX')}</p>
            </div>
            <div>
              <p className="text-sm text-slate-600 dark:text-slate-300">Dirección</p>
              <p className="font-semibold text-slate-900 dark:text-white">{order.address}</p>
            </div>
            {order.observaciones && (
              <div className="col-span-1 sm:col-span-2">
                <p className="text-sm text-slate-600 dark:text-slate-300">Observaciones del Pedido</p>
                <p className="font-semibold bg-red-100 dark:bg-red-900 p-2 rounded-lg text-slate-900 dark:text-white">{order.observaciones}</p>
              </div>
            )}
          </div>
          <div className="border-t pt-4 border-slate-200 dark:border-slate-700">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Productos</h3>
            {order.products.map((product) => (
              <div
                key={product.id}
                className={`border rounded-lg p-3 mb-2 ${statusStyles[product.status]?.border || statusStyles.Default.border} bg-slate-50 dark:bg-slate-800`}
              >
                <div className="flex justify-between items-center mb-2">
                  <p className="font-semibold text-slate-900 dark:text-white">{product.name}</p>
                  <p className="text-green-600 dark:text-green-400 font-bold">${product.subtotal.toFixed(2)}</p>
                </div>
                <div className="text-sm space-y-1">
                  <p className="text-slate-600 dark:text-slate-300">Cantidad: {product.quantity}</p>
                  <p className="text-slate-600 dark:text-slate-300">Precio Unitario: ${product.unitPrice.toFixed(2)}</p>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">Color: {product.color}</p>
                  <p>
                    Estado:{' '}
                    <span
                      className={`${
                  ...
                  {(product.status === 'Incompleto' || product.status === 'Incidente' || product.status === 'Faltante') && (
                    <>
                      <p className="text-slate-600 dark:text-slate-300">Cantidad Afectada: <span className="font-semibold">{product.issueQuantity || 0}</span></p>
                      <p className="text-slate-600 dark:text-slate-300">Observaciones: <span className="font-semibold">{product.observations || 'Ninguna'}</span></p>
                    </>
                  )}
                </div>
              </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800 p-3 flex justify-end border-t border-slate-200 dark:border-slate-700">
          <button onClick={onClose} className="px-4 py-2 bg-slate-200 dark:bg-slate-600 text-slate-800 dark:text-slate-200 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors">
            Cerrar
          </button>
        </div>
      </div>
...
const onCancel = async (pedidoId) => {
    try {
      ...
      const res = await api.put(`/api/pedidos/${pedidoId}/cancelar`, {}, {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    if (['Incompleto', 'Incidente'].includes(capitalizedStatus)) {
      ...
      try {
        const response = await api.put(`/api/repartidor/pedidos/${orderId}/status`, {
        ...
        setOrders((prev) =>
          prev.map((order) =>
            order.id === orderId ? { ...order, status: capitalizedStatus } : order
          )
        );
  ...
  const handleIncidentSubmit = async (orderId, { entireOrderIssue, orderObservations, productIssues, newStatus }) => {
    try {
      const capitalizedStatus = capitalizeStatus(newStatus);
      const payload = {
        entireOrderIssue,
      ...
      const response = await api.post(`/api/repartidor/pedidos/${orderId}/incidente`, payload, {
        withCredentials: true,
        headers: { 'X-CSRF-Token': csrfToken },
      });
      ...
      const updatedOrders = orders.map((order) =>
        order.id === orderId
          ? {
              ...
              observaciones: entireOrderIssue ? orderObservations : order.observaciones,
              products: entireOrderIssue
                ? order.products.map((p) => ({
                    ...p,
                    status: capitalizedStatus === 'Incidente' ? 'Incidente' : p.status,
                ...
                : order.products.map((p) => {
                    ...
                    return {
                      ...p,
                      status: issue ? capitalizeStatus(issue.estado) : p.status,
                      observations: issue && issue.estado !== 'Disponible' ? issue.nota : p.observations,
                    };
                  }),
            }
          : order
      ...
      if (updatedOrder.status === 'Incidente' || updatedOrder.status === 'Incompleto') {
              ...
              <div key={state} className="group">
                <div className="flex items-center mb-3 sm:mb-4">
                  <div className="flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-r from-amber-500 to-orange-600 rounded-full flex items-center justify-center shadow-lg">
                    <span className="text-white font-bold text-xs sm:text-sm">{stateIndex + 1}</span>
                  </div>
                ...
                <div className="space-y-2 sm:space-y-3 pl-2 sm:pl-4">
                  {Object.keys(groupedOrders[state]).map((municipality, municipalityIndex) => (
                    <div
                      key={municipality}
                    ...
                    >
                      <button
                        onClick={() => toggleMunicipality(state, municipality)}
                      ...
                      >
                        <div className="flex items-center min-w-0 flex-1">
                          <div className="w-4 h-4 sm:w-5 sm:h-5 bg-gradient-to-r from-yellow-500 to-orange-600 rounded-full flex items-center justify-center mr-2 sm:mr-3 shadow-md flex-shrink-0 ring-2 ring-amber-200 dark:ring-amber-700">
                            <span className="text-white font-bold text-xs">{municipalityIndex + 1}</span>
                          </div>
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 sm:mt-0 sm:ml-2 inline-flex">
                              <div className="px-2 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-amber-100 to-orange-100 dark:from-amber-900 dark:to-orange-900 text-amber-800 dark:text-amber-200 rounded-full text-xs font-bold shadow-sm">
                                {groupedOrders[state][municipality].length} pedidos
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="p-3 sm:p-4 lg:p-5 dark:from-gray-900/80 dark:to-amber-900/20 border-t border-amber-200 dark:border-gray-700">
                          {groupedOrders[state][municipality].length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
                              {groupedOrders[state][municipality].map((order) => (
                                <div key={order.id} className="transform hover:scale-105 active:scale-98 transition-transform duration-200 hover:z-10">
                                  <OrderCard
                                    order={order}
                            ...
                            <div className="text-center py-8 sm:py-10">
                              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-amber-200 to-orange-200 dark:from-gray-700 dark:to-gray-600 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-lg">
                                <span className="text-amber-600 dark:text-gray-300 text-lg sm:text-2xl">📦</span>
                              </div>
                              <NoDataDisplay message={`No hay pedidos en ${municipality}`} />
                            </div>
                          )}
                        </div>
                      )}
          ...
          <div className="text-center py-12 sm:py-16 lg:py-20">
            <div className="max-w-sm mx-auto px-4">
              <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-gradient-to-r from-amber-200 to-orange-300 dark:from-gray-700 dark:to-gray-600 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-xl ring-4 ring-amber-100 dark:ring-gray-600">
                <span className="text-amber-600 dark:text-gray-300 text-2xl sm:text-3xl lg:text-4xl">📋</span>
              </div>
