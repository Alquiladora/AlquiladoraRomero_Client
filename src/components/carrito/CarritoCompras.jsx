      ...
      <div className="relative">
        <div className="flex gap-5 overflow-x-auto no-scrollbar pb-4 snap-x snap-mandatory scroll-smooth">
          {productos.map((prod) => {
            ...
            if (!productId) {
              console.warn(
                'Recomendación omitida por falta de idProductoColores:',
                prod
              );
              return null;
            }
...
function CarritoRentaSheinStyle() {
  ...
  const calcularTotalCentralizado = (
    items,
    seleccionadosIds,
    fechaInicio,
    fechaFin,
  ...
  const handleCotizar = () => {
    ...
    const total = calcularTotalCentralizado(
      cartItems,
      selectedItems,
      rentalDate,
      returnDate,
  ...
  const filteredRecomendaciones = recomendaciones.filter(
    (rec) => !cartProductIds.includes(rec.idProducto)
  );
    ...
    const fetchGamificacionData = async () => {
      ...
      try {
        ...
        if (response.data.success && response.data.data) {
          ...
          setErrorNivel(
            response.data.message || 'Error al cargar datos de nivel.'
          );
        }
      } catch (err) {
  ...
  const getLevelInfo = (levelName) => {
    const discount = LEVEL_DISCOUNTS[levelName] || 0.0;
    ...
    if (discount === 0.1)
      benefitText = '10% de descuento + Prioridad en entregas';
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 text-gray-900 dark:text-gray-100">
      ...
      {showDetallesPago ? (
        <DetallesPago
          ...
          puntos={puntos}
          total={totalCalculated.toLocaleString('en-US', {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              {isLoading ? (
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-10 text-center">
                  <p className="text-red-500 dark:text-red-400 text-lg mb-6">
                    {error}
                  </p>
                  <button
                    onClick={fetchCartItems}
                    className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-6 py-3 rounded-lg hover:from-indigo-700 hover:to-indigo-800 transition shadow-md"
                  >
                    Reintentar
                  </button>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-16 text-center">
                  ...
                  <a
                    href="/cliente"
                    className="mt-6 inline-block bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-6 py-3 rounded-lg hover:from-indigo-700 hover:to-indigo-800 transition shadow-md"
                  >
                    Explorar productos
                  </a>
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <img
                        src={item.imagen}
                        alt={item.nombre}
                        className={`w-24 h-24 sm:w-28 sm:h-28 object-cover rounded-lg border ${isOutOfStock || isOverStock ? 'opacity-60' : ''}`}
                      />

                      <div className="flex-1 space-y-3 text-center sm:text-left">
                        {isOutOfStock ? (
                          <span className="out-of-stock text-sm">
                            Producto sin stock
                          </span>
                        ) : isOverStock ? (
                          <span className="out-of-stock text-sm">
                        ...
                        {remainingTime.showTimer &&
                          ...
                          !isOverStock && (
                            <span className="timer text-sm">
                              <ClockIcon className="h-5 w-5 mr-2" /> Tiempo
                              restante: {remainingTime.timeString}
                            </span>
                          )}
                        <p className="font-semibold text-lg sm:text-xl text-gray-900 dark:text-gray-100">
                          {item.nombre}
                        </p>

                        <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                          <span className="bubble text-sm">
                            <ColorSwatchIcon className="h-4 w-4 mr-1" /> Color:{' '}
                            {item.color}
                          </span>
                          <span className="bubble text-sm">
                            <CogIcon className="h-4 w-4 mr-1" /> Material:{' '}
                            {item.material}
                          </span>
                        </div>
                        ...
                        <div className="quantity-controls flex items-center justify-center sm:justify-start gap-4 mt-3">
                          ...
                          <span
                            className={`px-4 py-1 rounded-full font-medium text-sm ${isOutOfStock || isOverStock ? 'bg-gray-200 text-gray-500 dark:bg-gray-700 dark:text-gray-400' : 'bg-indigo-50 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200'}`}
                          >
                    ...
                    <div className="space-y-4 text-sm">
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-xs text-yellow-600 dark:text-yellow-400">
                          Nota: Si hoy es domingo, no puedes seleccionar ese
                          día. Si es viernes o sábado, puedes elegir domingo.
                        </p>
                      </div>
                        ...
                        <div className="mt-6 border-t pt-4 dark:border-gray-700">
                          ...
                          <div className="space-y-3 text-sm">
                            {(() => {
                              ...
                              const rawSubtotalDisplay =
                                subtotalAproximado * displayDays;
                              const discountAmountDisplay =
                                rawSubtotalDisplay * nivelData.discount;
                              const netAfterDiscount =
                                rawSubtotalDisplay - discountAmountDisplay;

                              const feeDisplay =
                                totalCalculated - netAfterDiscount;

                              const showBreakdown =
                                netAfterDiscount >= 150 ||
                                nivelData.name === 'Embajador de Fiesta';
                                ...
                                <>
                                    ...
                                    <>
                                      <div className="flex justify-between">
                                        ...
                                        <span className="font-medium text-gray-800 dark:text-gray-200 line-through">
                                          $
                                          {rawSubtotalDisplay.toLocaleString(
                                            'en-US',
                                            {
                                              minimumFractionDigits: 2,
                                              maximumFractionDigits: 2,
                                      ...
                                      <div className="flex justify-between text-green-600 dark:text-green-400 font-medium border-b border-dashed pb-2">
                                        <span>
                                          Descuento de Nivel (
                                          {(nivelData.discount * 100).toFixed(
                                            0
                                          )}
                                          %) :
                                        </span>
                                        <span>
                                          -$
                                          {discountAmountDisplay.toLocaleString(
                                            'en-US',
                                            {
                                              minimumFractionDigits: 2,
                                              maximumFractionDigits: 2,
                                  ...
                                  {!showBreakdown && rawSubtotalDisplay > 0 && (
                                    <div className="flex justify-between text-indigo-600 dark:text-indigo-400 text-xs italic mb-2">
                                      <span>
                                        * Tarifa mínima de servicio aplicada
                                      </span>
                                  ...
                                  {nivelData.discount > 0 && showBreakdown && (
                                    <div className="flex justify-between text-gray-500 dark:text-gray-400 text-sm mb-1">
                                      ...
                                      <span>
                                        $
                                        {netAfterDiscount.toLocaleString(
                                          'en-US',
                                          {
                                            minimumFractionDigits: 2,
                                            maximumFractionDigits: 2,
                                  ...
                                  {showBreakdown && feeDisplay > 0 && (
                                    <div className="flex justify-between text-gray-500 dark:text-gray-400 text-sm border-b border-gray-200 dark:border-gray-700 pb-2 mb-2">
                                      ...
                                      <span>
                                        +$
                                        {feeDisplay.toLocaleString('en-US', {
                                          minimumFractionDigits: 2,
                                          maximumFractionDigits: 2,
                                        })}
                                      </span>
                                  ...
                                  <div className="flex justify-between font-semibold">
                                    ...
                                    <span className="text-gray-900 dark:text-white">
                                      $
                                      {totalCalculated.toLocaleString('en-US', {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2,
                                      })}
                                    </span>
          ...
          <SeccionRecomendaciones
            productos={filteredRecomendaciones}
            onAgregar={addToCartt}
