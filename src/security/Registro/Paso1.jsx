...
const Paso1 = ({ onValidationSuccess, setGuardarCorreo }) => {
  ...
  const handleValidation = async (e) => {
    ...
    if (!executeRecaptcha) {
      showAlert("error", "reCAPTCHA no cargado correctamente. Intenta nuevamente.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const usuarioCorreo = usuarios.some(
        (usuario) => usuario.correo.toLowerCase().trim() === email.toLowerCase().trim()
      );
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white shadow-md rounded-lg dark:bg-gray-950 dark:text-white relative">
