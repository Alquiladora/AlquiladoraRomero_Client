...
const CambiarPassword = () => {
  ...
  const getErrorColor = (message) => {
    ...
    if (message.includes("registrado") || message.includes("problema")) return "bg-red-100 text-red-700";
  ...
  const handleEmailSubmit = async (e) => {
    ...
    const isBloqueado = await verificarCambiosContrasena(usuarioEncontrado.idUsuarios);
    if (isBloqueado) {
      setErrorMessage("Has alcanzado el límite de cambios de contraseña para este mes. Por favor, intenta el próximo mes.");
      setLoading(false);
      return;
    }
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              ...
              <button
                ...
                className={`w-full flex justify-center items-center bg-blue-600 text-white py-2 px-4 rounded-md ${
                 loading || bloqueado ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-700"
                }`}
